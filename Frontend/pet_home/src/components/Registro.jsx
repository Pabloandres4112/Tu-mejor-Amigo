...
function RegistroPets() {
  const [formData, setFormData] = useState({
    ...
    fk_categories: '',
    gender_id: '',
    user_id: '8', // Asigna el user_id de manera estática o dinámica según sea necesario
  });
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('user_id', formData.user_id); // Asegúrate de incluir user_id
    if (file) {
    ...
    try {
      ...
      if (response.data.message === 'Mascota registrada correctamente') {
        navigate('/home');
      }
    } catch (error) {
